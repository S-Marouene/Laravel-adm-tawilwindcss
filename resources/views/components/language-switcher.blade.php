...
<div x-data="localeSwitcher()" class="relative">
    @if($variant === 'flag')
        ...
        <button @click="open = !open" id="dropdown-flag" type="button" class="hs-dropdown-toggle ti-dropdown-toggle !p-0 flex-shrink-0 !border-0 !rounded-lg !shadow-none inline-flex cursor-pointer items-center justify-center w-9 h-9 text-gray-500 dark:text-gray-400 hover:text-gov-600 dark:hover:text-gov-400 hover:bg-gray-100 dark:hover:bg-gray-700/60 transition-all duration-200">
        ...
        <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="main-header-dropdown absolute end-0 mt-2 min-w-[10rem]" style="display: none;">
            <div class="py-2">
                <button @click="switchLocale('fr')" type="button" class="ti-dropdown-item !p-[0.65rem] w-full cursor-pointer" :class="currentLocale === 'fr' ? 'bg-gov-50 dark:bg-gov-900/20 text-gov-700 dark:text-gov-300' : ''">
                ...
                <button @click="switchLocale('ar')" type="button" class="ti-dropdown-item !p-[0.65rem] w-full cursor-pointer" :class="currentLocale === 'ar' ? 'bg-gov-50 dark:bg-gov-900/20 text-gov-700 dark:text-gov-300' : ''">
                    <div class="flex items-center gap-3 w-full">
                        <span class="text-lg leading-none">🇹🇳</span>
    ...
    @else
        ...
        <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-100" x-transition:enter-start="transform opacity-0 scale-95" x-transition:enter-end="transform opacity-100 scale-100" x-transition:leave="transition ease-in duration-75" x-transition:leave-start="transform opacity-100 scale-100" x-transition:leave-end="transform opacity-0 scale-95" class="absolute mt-2 w-48 rounded-xl bg-white dark:bg-gray-800 shadow-lg border border-gray-200 dark:border-gray-700 ring-1 ring-black/5 z-50" :class="currentLocale === 'ar' ? 'left-0' : 'right-0'" style="display: none;">
            <div class="py-1">
                <button @click="switchLocale('fr')" type="button" class="w-full flex cursor-pointer items-center gap-3 px-4 py-3 text-sm transition-colors" :class="currentLocale === 'fr' ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 font-semibold' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700/50'">
                ...
                <button @click="switchLocale('ar')" type="button" class="w-full flex cursor-pointer items-center gap-3 px-4 py-3 text-sm transition-colors" :class="currentLocale === 'ar' ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 font-semibold' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700/50'">
                    <span class="text-lg">🇹🇳</span>
                    <div class="text-left" :dir="'ltr'">
