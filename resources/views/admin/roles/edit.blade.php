...
@section('content')
    <div class="mb-6">
        ...
        <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-2">Edit Role: {{ $role->name }}</h1>
    </div>

    <div class="max-w-2xl">
        <form action="{{ route('admin.roles.update', $role) }}" method="POST" class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 space-y-6">
            @csrf
            @method('PATCH')
                        ...
                        <div class="grid grid-cols-2 gap-2">
                            @foreach($groupPermissions as $permission)
                                <label class="inline-flex items-center p-2 rounded hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    <input type="checkbox" name="permissions[]" value="{{ $permission->id }}" {{ in_array($permission->id, $rolePermissions) ? 'checked' : '' }} class="rounded border-gray-300 text-purple-600 shadow-sm focus:ring-purple-500">
            ...
            <div class="flex items-center justify-end space-x-3">
                ...
                <button type="submit" class="px-4 py-2 bg-purple-600 hover:bg-purple-700 text-white text-sm font-medium rounded-lg transition-colors">Update Role</button>
            </div>
        </form>
    </div>
@endsection
